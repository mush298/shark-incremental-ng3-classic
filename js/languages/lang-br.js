...
LANGUAGES.BR = {
    ...
    text: {
        ...
        'fish-name': "red fish blue fish one fish two fish",
        'fish-costName': toTextStyle('red fish blue fish one fish two fish','fish'),

        'prestige-name': "sussy shards",
        'prestige-costName': toTextStyle('sussy','prestige') + " shards",

        'coral-name': "ocean tubes",
        'coral-costName': toTextStyle('ocean tubes','coral'),

        'ice-name': "cold water",
        'ice-costName': toTextStyle('cold water','ice'),

        'salt-name': "skibussy",
        'salt-costName': toTextStyle('skibussy','salt'),

        'snow-name': "Snow flakes or something smh",
        'snow-costName': toTextStyle('Snow flakes or something smh','snow'),

        'kelp-name': "weird ocean plants",
        'kelp-costName': toTextStyle('Kelp','kelp'),

        'core-name': "core currency",
        'core-costName': toTextStyle('core currency','core'),

        'humanoid-name': "shark men",
        'humanoid-costName': toTextStyle('shark men','humanoid'),

        'remnants-name': "purple things",
        'remnants-costName': toTextStyle('purple things','black-hole'),

        'dark-matter-name': "purple things 2",
        'dark-matter-costName': toTextStyle('Dark Matter','black-hole'),

        'observ-name': "look things",
        'observ-costName': toTextStyle('look things','observ'),

        'reserv-name': "reservation things",
        'reserv-costName': toTextStyle('reservation things','reserv'),

        'traject-name': "traject things",
        'traject-costName': toTextStyle('traject things','traject'),

        'infinity-name': "unlimited points",
        'infinity-costName': toTextStyle('unlimited points','infinity'),

        'full-shark-level': toTextStyle('social credit','shark'),
        'full-shark-rank': toTextStyle('social credit 2','shark'),

        'sharkoid-faith': toTextStyle('shark man charisma','humanoid'),

        'curr-top-0-req': x => `Reach <b>${format(x)}</b> of ${toTextStyle('ocean creatures','fish')} to do this!!!!!!!!!`, 
        'curr-top-0-reset': x => `Sus (sus) for <b>${x.format(0)}</b> ${toTextStyle('Sussy','prestige')} shards?!?!?!!?!?`,

        'curr-top-1-req': x => `Reach <b>${format(x)}</b> of da ${toTextStyle('Sussy','prestige')} shards?!?!?!!?!?`, 
        'curr-top-1-reset': x => `Core (core) for <b>${x.format(0)}</b> ${toTextStyle('core currency','core')}!!!!!!`,

        'curr-top-2-req': x => `Reach <b>${format(x)}</b> of ${toTextStyle('ocean creatures','fish')} to do this!!!!!!!!!`, 
        'curr-top-2-reset': (x,next) => `become <b>${format(x,0)}</b> ${toTextStyle('shark man','humanoid')} (next at <b>${format(next)}</b> ${toTextStyle('fis','fish')}!!!!!!!!!!!!`,
        ...
        'infinity-upgrades': [
          ...
          `<div>
           ...
           (x, y)=>
            `<div>
             <h3>No Hardcap</h3>
             <p>Removes the hardcap at <b>${format(x)}</b> ${toTextStyle('Fish','fish')}. Unlock a new ${toTextStyle("Shark",'shark')} upgrade.</p>
             <p>Cost: <b>${format(y)}</b> ${toTextStyle('Infinity Points','infinity')} </p>
             </div>`,
             ...
             (x, y)=>
                `<div>
                 ...
                 <p>Cost: <b>${format(y)}</b> Infinity Points, 3 ${toTextStyle("Infinities",'infinity')} </p>
                 </div>`,

            (x, y)=>
            `<div>
            <h3>No Overflow</h3>
            <p>Removes the overflow at <b>${format(x)}</b> ${toTextStyle('Fish','fish')}. Remove the reset animation.</p>
            <p>Cost: <b>${format(y)}</b> Infinity Points, 3 ${toTextStyle("Infinities",'infinity')} </p>
            </div>`
        ],
