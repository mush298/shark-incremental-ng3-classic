...
const ST_NAMES = [
    ...
    null,
		...
		["","Ki","Mg","Gi","Tr","Pt","Ex","Zt","Yt","Xe"],
		["","He","Do","Tr","Te","Pe","Ex","Ze","Yo","Ne"],
		["","Dak","Ik","Trk","Tek","Pek","Exk","Zek","Yok","Xen"],
		["","Ho","Bo","Tro","To","Po","Exo","Zo","Yoo","Non"]
	],
]
const ST_NAMES_LONG = [
	null, [
    ["","million","billion","trillion","quadrillion","quintillion","sextillion","septillion","octillion","nonillion"],
		["","un","duo","tre","quattour","quin","sex","septen","octo","novem"],
		...
		["","me","du","trio","tetre","pente","hexe","hepte","octe","enne"],
...
const FORMATS = {
    ...
    standard: {
      tier1(x) {
        return ST_NAMES[1][1][x % 10] + ST_NAMES[1][2][Math.floor(x / 10) % 10] + ST_NAMES[1][3][Math.floor(x / 100)] 
      },
      tier2(x) {
        let o = x % 10
      ...
      standard_long: {
        tier1(x) {
          if (x < 10) {
          return ST_NAMES_LONG[1][0][x]
        } else if (x < 100) {
          return ST_NAMES_LONG[1][1][x % 10] + ST_NAMES_LONG[1][2][Math.floor(x / 10)]
        } else if (x < 1000) {
          let i = ""
          if ((Math.floor(x / 10) % 10) !== 0) i = "i"
          return ST_NAMES_LONG[1][1][x % 10] + ST_NAMES_LONG[1][2][Math.floor(x / 10) % 10] + i + ST_NAMES_LONG[1][3][Math.floor(x / 100)] 
        }
    ...
    switch (type) {
        case "sc":
            if (ex.log10().lt(Math.min(-acc,0)) && acc > 1) {
                ...
                return neg+(a>0?ex.toFixed(a):ex.toNumber().toFixed(a).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'))
            } else {
                if (ex.gte("F10")) {
                    let slog = ex.slog()
        ...
        case "st":
            if (e.lt(max) || ex.gte("F10")) return format(ex, acc, max, "sc")
            let final = ""
            ...
            if (e3.lt(1)) {
              ...
              if (ee.gte(3000)) {
                let eee = ee.log10().floor()
                if (eee.gte(3000)) {
                  return "e" + format(ex.log10(), acc, max, "st")
                }

                e3 = ex.log(1e3).log(1e3).floor()
                let ee3 = Math.floor(ee.log(1e3).toNumber())
                ...
                while (e3.gt(0)) {
                  ...
                  if (mod1000 > 0) {
                    if (mod1000 == 1 && !ee3) final = "Mi"
                    if (ee3) final = FORMATS.standard.tier3(ee3) + (final ? "-" + final : "")
                    if (mod1000 > 1) final = FORMATS.standard.tier2(mod1000) + final
                  }
                  e3 = div1000
                  ee3++
                }
                ...
                //format('ee3300', 2, 15, "st")

              

              
              if (e3.lt(9)) final = ["", "K", "M", "B","T","Qa","Qi","Sx","Sp","Oc","No"][Math.round(e3.toNumber())]
              else {
        ...
        case "omega":
        if (ex.gte("F10")) return format(ex, acc, max, "sc")
        return FORMATS["omega_short"].format(ex, 3, max)

        case "layer":
          if (ex.gte("F10")) return format(ex, acc, max, "sc")
            return FORMATS["layer"].format(ex, 3, max)

          case "elemental":
          if (ex.gte("F10")) return format(ex, acc, max, "sc")
            return FORMATS["elemental"].format(ex, 3)

          case "inf":
            if (ex.gte("F10")) return format(ex, acc, max, "sc")
              return FORMATS["inf"].format(ex, 3)
...
Decimal.prototype.format = function (acc,  max=options.max_range) { return format(this.clone(), acc, max) }
