...
function updateHTML() {
    ...
    if (f.gte(tmp.corrupt_start)) ff.push(toTextStyle(icon("warning", 'corruption')))
    ...
    el('next-infinity').style.display = el_display(player.inf.infinities.gte(1))

    el('next-infinity').innerHTML = lang_text('next-infinity', format(INFINITY.gain(player.inf.infinities.add(1))))
...
function setupTopCurrenciesHTML() {
    ...
    for (let [i,x] of Object.entries(TOP_CURR)) {
        ...
        <div class="curr-top" id="curr-top-${i}-div">
            <div id="curr-top-${i}-amt1"><span id="curr-top-${i}-amt2">???</span> ${CURRENCIES[x.curr].costName} <div id="curr-top-${i}-amt3"><span id="curr-top-${i}-amt4">???</span> ${toTextStyle('Infinities', 'infinity')}</div></div><button class = ${x.style} onclick="doReset('${x.reset ?? x.curr}')" id="curr-top-${i}-btn">Reset</button>
        </div>
        `
        } else {
            h += `
...
function updateTopCurrenciesHTML() {
    for (let [i,x] of Object.entries(TOP_CURR)) {
        ...
        el(`curr-top-${i}-btn`).className = el_classes({locked: !req, core: style == "core", evo: style == "evo", bh: style == "bh", inf: style == "inf"})

        if (TOP_CURR[i]?.inf) {
            el(`curr-top-${i}-amt4`).innerHTML = format(player.inf.infinities, 0) + (tmp.infinity_gain.gte(1) ?` (+${format(tmp.infinity_gain, 0)})`:"")
        } 
    }
}
