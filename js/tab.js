...
const TAB_IDS = {
    ...
    'infinity-upgrades': {
        html: updateInfinityHTML,
    },
    'infinity-power': {
        html: updateInfinityHTML,

        notify() {
            if (player.fish.gte(INFINITY.ordinal_cost(player.inf.ordinal + 1)) && player.inf.ordinal < 23) return true;
        },
    },
    'corruption': {
        html: updateInfinityHTML,
...
const TABS = [
    ...
    {
        unl: ()=>player.inf.infinities.gte(16),
        stab: "elements",
        style: {
            "background": `black`,
            "color": "white",
        ...
        style: {
            background: `repeating-conic-gradient(
                rgba(145, 10, 10, 0.1) 0deg, 
                rgba(145, 10, 10, 0.1) 25%, 
    ...
    {
        id: 'infinity',
        unl: ()=>player.inf.infinities.gte(1),
        stab: [
            ["infinity-upgrades"],
            ["infinity-power",()=>hasInfinityUpgrade(2)],
            ["corruption",()=>hasInfinityUpgrade(11)]
        ],
        tab_style: "inf",
        style: {
            "background": `black url('textures/infinity-pattern.png')`,
            "color": "white",
            "animation": `cosmic-pattern 20s linear infinite`,
        },
    },
...
function updateTabs() {
    ...
    TABS[8].style.background = player.radios.potato == 1 ? `#FF6961`: `repeating-conic-gradient(
        rgba(145, 10, 10, 0.1) 0deg, 
        rgba(145, 10, 10, 0.1) 25%, 
        rgba(255, 0, 0, 0.4) 0deg, 
        rgba(255, 0, 0, 0.6) 50%
    ...
    TABS[9].style.background = player.radios.potato == 1 ? `#77dd77`: `repeating-conic-gradient(
    rgba(0, 255, 3, 0.1) 0deg, 
    rgba(0, 255, 3, 0.1) 25%, 
    rgba(0, 200, 50, 0.4) 0deg, 
     rgba(0, 150, 30, 0.6) 50%
    ...
    for (let [i,v] of Object.entries(TABS)) {
        ...
        if (array) {
            ...
            if (selected) v.stab.forEach(([x,u],j) => {
                ...
                s_elem.className = el_classes({ [`${TABS[i].tab_style} tab-button`]: true, stab: true, selected: x == tab_name, notify: tab_unlocked[i].includes(x)}) // "tab-button stab"+(x == tab_name ? " selected" : "")
            })
        }
        ...
        if (unl) elem.className = el_classes({[`${TABS[i].tab_style} tab-button`]: true, selected, notify: player.radios.notify && (array ? tab_unlocked[i].length > 0 : getTabNotification(v.stab))}) // "tab-button"+(selected ? " selected" : "")
    }
