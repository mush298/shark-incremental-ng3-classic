const CORE_RAD = {
    ...
    gain() {
        ...
        if (hasInfinityUpgrade(4)) x = expPow(x, infUpgradeEffect(4))
    ...
    boosts: [
        ...
        },
        {
            req: 813000000,
            effect: (r,b)=>{
                let x = r.add(10).log10().log10().add(1).log10().pow(0.25).mul(b.log10().pow(0.25)).pow(0.5).mul(getCRBoost(13))
...
function updateCoreRadiation() {
    ...
    el('upgrade-cr-btn').className = el_classes({locked: !r_c15 && !active || CURRENCIES.fish.amount.pow(!active && r_c15 ? researchEffect('c15',0) : 1).lt(cost), 'core huge-btn': true})

    el('reset-cr-btn').innerHTML = lang_text('reset-cr',CORE_RAD.limitIncrease())
    el('reset-cr-btn').className = el_classes({locked: rad.amount.lt(tmp.cr_limit), 'core huge-btn': true})
