const RESETS = {
    ...
    'black-hole': {
        ...
        doReset() {
            ...
            if (player.inf.infinities < 0) ores_grid = []
    ...
    infinity: {
        get require() { return player.fish.gte('ee9e15') },
        reset(force) {
            if (!force) {
                increaseFeature(21)
                gainCurrency('infinity',tmp.currency_gain['infinity'])

                if (player.inf.infinities.gte(16))  gainCurrency('quarks',tmp.currency_gain['quarks'])
                if (tmp.infinity_gain > -1) player.inf.infinities = player.inf.infinities.add(tmp.infinity_gain)
            }
        ...
        doReset() {
            ...
            if (!hasInfinityUpgrade(11)) player.singularity.bh_tier = E(1)

            for (let i = 0; i < 4; i++) {
                player.singularity.constellation_res[i] = E(0)
            }
