...
const REMNANT_UPGS = [
    ...
    {
        unl:()=>hasElement(4),
        cost:a=>a.pow_base(1.1).pow_base('ee22'),
        bulk:a=>a.log('ee22').log(1.1),
        effect(a) {
            let x = a.add(1).pow(0.01)
            if (hasElement(21)) x = x.sub(1).mul(2).add(1)
            return x
        },
        effDesc: x=>formatPow(x),
    },
    {
        unl:()=>hasElement(16),
        cost:a=>a.pow_base(1.25).pow_base('e5e29'),
        bulk:a=>a.log('e5e29').log(1.25),
        effect(a) {
            let x = a
            if (hasElement(20)) x = x.pow(1.25)
            return x
        },
        effDesc: x=>"+"+format(x,0),
    },
...
function updateSingularityTemp() {
    tmp.bh_reduction = player.inf.corrupt ? E('1e-10') : tmp.ss_difficulty ? E(.5) : player.singularity.bh.gte(8) ?  hasElement(18) ? E(1).add(player.singularity.bh.sub(8)).pow(hasElement(22) ? (hasElement(29) ? E(5).add(hasElement(30) ? player.elements.length / 15 : E(0)) : 2.5) : 1.5) : E(1).add(player.singularity.bh.sub(8)) : Decimal.div(1,player.singularity.bh.sub(player.research.dm1).max(0).pow(2).div(20).add(1))
2
...
function updateBlackHoleHTML() {
    ...
    el('black-hole-amount').innerHTML = format(player.singularity.bh,0) + ` / ${format(tmp.bh_limit,0)}`
