...
const SHARK_UPGRADES = {
    ...
    s6: {
        unl: ()=>hasInfinityUpgrade(0),
        req: ()=>hasInfinityUpgrade(1),

        cost: l => {
            let x = Decimal.pow(1.01, l.scale(10, 3, 'E2')).mul(16).pow10().pow10().pow10()
            return x
        },
        bulk: x => {
            return x.log10().log10().log10().div(16).log(1.01).scale(10, 3, 'E2', true).floor().add(1)
        },
    ...
    p1: {
        ...
        effect: l=>Decimal.pow(3,l.mul(simpleResearchEffect('p5'))).min(hasElement(2) ? EINF : 'eee7').overflow('eee7',0.5,2),
    ...
    p2: {
        ...
        effect: l=>Decimal.pow(player.fish.add(10).max(10).log10(),l).min(hasElement(5) ? EINF : 'eee7').overflow('eee7',0.5,2),
        effDesc: x=>formatMult(x),
    },
    p3: {
        unl: ()=>hasResearch('p3'),
...
const SU_TABS = {
    'shark': ['s1','s2','s3','s4','s5','s6', 'p1','p2','p3'],
...
function updateSharkUpgradesHTML() {
    ...
    for (let [i,v] of Object.entries(SHARK_UPGRADES)) {
       ...
       if (v.style == "evo") {
        el('su-'+i+'-cost').className = el_classes({'evo shark-upgrade-button': true, locked: tmp.su_locked[v.curr] || CURRENCIES[v.curr].amount.lt(cost)})
       } else {
    ...
    for (let [i,v] of Object.entries(SHARK_UPGRADES)) {
        ...
        <div class="shark-upgrade" id="shark-upgrade${i}-div">
            <div class="shark-upgrade-div" id="su-${i}-info">
                ...
                <div class="shark-upgrade-buttons"><button id="su-${i}-cost" onclick="buySharkUpgrade('${i}')">Cost: ???</button><button class = "${v.style}" onclick="buyMaxSharkUpgrade('${i}')">${lang_text('buyMax')}</button></div>
            </div>
