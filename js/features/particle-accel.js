...
function setupPAHtml() {
    ...
    for (let i = 0; i < PARTICLE_ACCELERATOR.length; i++) {
        ...
        <div id="particle-accel-${i}-div">
            <button id="particle-accel-${i}-button" class="evo particle-accel-button" onclick="player.humanoid.particle_accel.active = ${i}"></button>
...
function updatePAHtml() {
    ...
    if (unl) {
        for (let i = 0; i < PARTICLE_ACCELERATOR.length; i++) {
            ...
            el(`particle-accel-${i}-button`).className = el_classes({ active : player.humanoid.particle_accel.active == i, 'evo particle-accel-button' : true })
