const CONSTELLATION = {
    get exp() {
        ...
        if (hasInfinityUpgrade(9)) x = x.mul(infUpgradeEffect(9))
        return x
    },
    ...
    boosts: [
        {
            ...
            req: 2,
            effect(r,d) {
                let x = d ? expPow(r.max(0),0.5).div(25).add(1) : hasInfinityUpgrade(5) ? r.add(1).log10().div(4).add(1) : r.min(1e28).add(1).log10().div(4).add(1)
                return x
            },
            effDesc: x=>formatPow(x,3),
        },{
            ...
            req: 4,
            effect(r,d) {
                let x = d ? expPow(r.max(0),1/3).div(100).add(1) : hasInfinityUpgrade(5) ? expPow(r.add(1).log10(),2).pow_base(10) : expPow(r.min(1e26).add(1).log10(),2).pow_base(10)
                return x
            },
            effDesc: x=>formatPow(x,3),
        },{
            ...
            req: 8,
            effect(r,d) {
                let x = d ? r.add(1).log10().root(2).div(10).add(1).pow(-1) : hasInfinityUpgrade(5) ? r.add(1).log10().div(10).add(1) : r.min(1e22).add(1).log10().div(10).add(1)
                return x
            },
...
function updateConstellationHTML() {
    ...
    el('bh-tier-button').className = el_classes({locked: base.lt(req), 'bh half-huge-btn': true})
