...
function updateEvolutionTreeHTML() {
    ...
    for (let x = 0; x < EVOLUTION_TREE.rows*4; x++) {
        ...
        tree_el.className = el_classes({locked: c ? !EVOLUTION_TREE.canAfford(x, charged_row_available[row], true) : !EVOLUTION_TREE.canAfford(x, row_available[row]), bought: !c && player.humanoid.tree.includes(x), 'pre-charged': c, charged: player.humanoid.tree.includes(x+"C"), "evo evolution-tree-btn": true})
    ...
    for (let [i,j] of Object.entries(EVOLUTION_TREE.faith_cost)) {
        ...
        el_btn.className = el_classes({locked: curr.amount.lt(cost), 'evo huge-btn': true})
    }
...
function updateEvolutionGoalHTML() {
    ...
    for (let i = 0; i < EVOLUTION_GOAL.length; i++) {
        ...
        el_ctn.className = el_classes({locked: !goal && locked, bought: goal, 'evo evolution-goal-ctn': true})
